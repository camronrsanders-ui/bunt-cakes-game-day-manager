(function(){
  ...
  function renderCard(){
    ...
    if(!date){
      card.innerHTML='<div class="attendance-head"><div><div class="muted">WEEKLY GAME CHECK-IN</div><h2 style="margin:.25rem 0">Sunday Availability</h2><div class="muted">No upcoming Sunday game is posted yet.</div></div></div>'+(paired?notificationBlock():accessRequiredBlock());
      bindCard();return;
    }
    const games=gamesFor(date),answer=paired?responseFor(date,name):'';
    const gameText=games.map(g=>'<div><strong>'+time12(g.time)+'</strong> — '+esc(g.title||'Game')+'</div>').join('');
    card.innerHTML=`<div class="attendance-head"><div><div class="muted">WEEKLY GAME CHECK-IN</div><h2 style="margin:.25rem 0">Will you be there Sunday?</h2><div>${esc(prettyDate(date))}</div></div><span class="attendance-badge">LINEUP RSVP</span></div><div class="attendance-games">${gameText}</div>${paired?'<div><strong>'+esc(name)+', choose your answer:</strong></div><div class="attendance-answer"><button data-answer="yes" class="'+(answer==='yes'?'on':'')+'">✅ Yes</button><button data-answer="no" class="no '+(answer==='no'?'on':'')+'">❌ No</button><button data-answer="not_sure" class="maybe '+(answer==='not_sure'?'on':'')+'">🤔 Not sure</button></div>':accessRequiredBlock()}${answer?'<div class="attendance-saved">Saved: '+statusLabel(answer)+' • Captain View updates automatically.</div>':''}${paired?notificationBlock():''}`;
    bindCard();
  ...
  async function refreshPushStatus(){
    ...
    if(!playerName()){text.textContent='Player access is required before notifications can be connected.';btn.textContent='Player access required';btn.disabled=true;return}
