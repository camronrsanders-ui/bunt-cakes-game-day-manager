(function(){
  ...
  if(!document.getElementById(STYLE_ID)){
    ...
    style.textContent=`
      ...
      .attendance-player{margin-top:9px}.attendance-notify{margin-top:10px;padding-top:10px;border-top:1px solid #bbf7d0}.attendance-notify button{width:100%;font-weight:800}.attendance-saved{font-weight:800;color:#166534;margin-top:8px}
  ...
  function sundayDates(){const t=today(),events=Array.isArray(window.state?.events)?state.events:[];return[...new Set(events.filter(e=>e&&e.type==='Game'&&e.date&&e.date>=t).map(e=>e.date).filter(d=>new Date(d+'T12:00:00').getDay()===0))].sort()}
  ...
  function renderCard(){
    if(typeof state==='undefined'||!state)return;const card=mount();if(!card)return;const date=targetDate();
    ...
    card.innerHTML=`<div class="attendance-head"><div><div class="muted">WEEKLY GAME CHECK-IN</div><h2 style="margin:.25rem 0">Will you be there Sunday?</h2><div>${esc(prettyDate(date))}</div></div><span class="attendance-badge">LINEUP RSVP</span></div><div class="attendance-games">${gameText}</div>${name?'<div><strong>'+esc(name)+', choose your answer:</strong></div>':'<div><strong>Choose your name first:</strong></div>'}${name?`<div class="attendance-answer"><button data-answer="yes" class="${answer==='yes'?'on':''}">✅ Yes</button><button data-answer="no" class="no ${answer==='no'?'on':''}">❌ No</button><button data-answer="not_sure" class="maybe ${answer==='not_sure'?'on':''}">🤔 Not sure</button></div>`:`<div class="attendance-player"><select id="attendancePlayer">${playerOptions(name)}</select></div>`}${answer?'<div class="attendance-saved">Saved: '+statusLabel(answer)+' • Captain View updates automatically.</div>':''}<div class="attendance-notify"><div id="attendanceNotifyText" class="muted">Turn on Thursday reminders so you do not have to remember to check the app.</div><button id="enableAttendancePush" style="margin-top:8px">Enable Thursday Push Reminder</button></div>`;
    const sel=document.getElementById('attendancePlayer');if(sel)sel.onchange=()=>setPlayer(sel.value);card.querySelectorAll('[data-answer]').forEach(btn=>btn.onclick=()=>saveAnswer(btn.dataset.answer));const push=document.getElementById('enableAttendancePush');if(push)push.onclick=enablePush;refreshPushStatus();if(new URLSearchParams(location.search).get('availability')===date)setTimeout(()=>card.scrollIntoView({behavior:'smooth',block:'center'}),150)
  }
  ...
  async function refreshPushStatus(){const text=document.getElementById('attendanceNotifyText'),btn=document.getElementById('enableAttendancePush');if(!text||!btn)return;if(!('Notification'in window)||!('PushManager'in window)){text.textContent='This browser does not support web push reminders.';btn.disabled=true;return}const isIOS=/iphone|ipad|ipod/i.test(navigator.userAgent);if(isIOS&&!standalone()){text.textContent='On iPhone, add this team app to your Home Screen first. Then open it and enable notifications.';btn.textContent='Home Screen app required';btn.disabled=true;return}try{const reg=await registration(),sub=await reg.pushManager.getSubscription();if(sub&&Notification.permission==='granted'){text.textContent='Thursday push reminders are ON for this phone.';btn.textContent='Thursday Reminders On';btn.disabled=true;await postSubscription(sub);return}}catch(e){}text.textContent='Get a push notification each Thursday when a Sunday game is scheduled.';btn.textContent='Enable Thursday Push Reminder';btn.disabled=false}
  async function enablePush(){const name=playerName();if(!name){alert('Choose your name first.');return}const btn=document.getElementById('enableAttendancePush');if(btn){btn.disabled=true;btn.textContent='Turning on…'}try{if(!('Notification'in window)||!('PushManager'in window))throw new Error('Push notifications are not supported in this browser');const permission=await Notification.requestPermission();if(permission!=='granted')throw new Error('Notifications were not allowed');const config=await fetch('/api/team-state?pushConfig=1',{cache:'no-store'}).then(async r=>{const j=await r.json();if(!r.ok)throw new Error(j.error||'Could not load push setup');return j});const reg=await registration();let sub=await reg.pushManager.getSubscription();if(!sub)sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:b64ToBytes(config.publicKey)});await postSubscription(sub);await refreshPushStatus()}catch(e){alert(e.message||'Could not enable reminders');if(btn){btn.disabled=false;btn.textContent='Enable Thursday Push Reminder'}}}

  const wait=setInterval(()=>{if(typeof state==='undefined'||!state||!document.getElementById('home'))return;clearInterval(wait);renderCard();syncExistingSubscription();setInterval(renderCard,3000)},250);window.addEventListener('pageshow',renderCard);
})();
