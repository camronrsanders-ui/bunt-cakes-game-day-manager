(function(){
  ...
  if(!document.getElementById(STYLE_ID)){
    ...
    style.textContent=`
      ...
      .attendance-badge{display:inline-block;border-radius:999px;padding:5px 9px;background:#15803d;color:#fff;font-size:.76rem;font-weight:900}
      ...
      .attendance-answer button{font-weight:900;background:#fff}
      .attendance-answer button.on{background:#15803d;color:#fff;border-color:#15803d}
      .attendance-answer button.no.on{background:#b91c1c;border-color:#b91c1c}
      .attendance-answer button.maybe.on{background:#a16207;border-color:#a16207}
      .attendance-player{margin-top:9px}
      .attendance-notify{margin-top:10px;padding-top:10px;border-top:1px solid #bbf7d0}
      .attendance-notify button{width:100%;font-weight:800}
  ...
  function playerName(){return new URLSearchParams(location.search).get('player')||localStorage.getItem('buntCakesPlayerName')||''}
  function setPlayer(name){
    if(!name)return;
    localStorage.setItem('buntCakesPlayerName',name);
    const u=new URL(location.href);u.searchParams.set('player',name);history.replaceState(null,'',u.pathname+u.search+u.hash);
    renderCard();syncExistingSubscription();
  }
  function sundayDates(){
    const t=today();
    const events=Array.isArray(window.state?.events)?state.events:[];
    return [...new Set(events.filter(e=>e&&e.type==='Game'&&e.date&&e.date>=t).map(e=>e.date).filter(d=>new Date(d+'T12:00:00').getDay()===0))].sort();
  }
  function targetDate(){
    const requested=new URLSearchParams(location.search).get('availability');
    const dates=sundayDates();
  ...
  function playerOptions(current){
    const players=(state?.players||[]).slice().sort((a,b)=>(a.fullName||a.name).localeCompare(b.fullName||b.name));
    return '<option value="">Choose your name</option>'+players.map(p=>'<option value="'+esc(p.name)+'" '+(p.name===current?'selected':'')+'>'+esc(p.fullName||p.name)+'</option>').join('');
  }

  function mount(){
    const home=document.getElementById('home');
    if(!home)return null;
    let card=document.getElementById('weeklyAttendanceCard');
    if(!card){
      card=document.createElement('div');card.id='weeklyAttendanceCard';card.className='card attendance-card';
      const reminder=home.querySelector('.reminder-card');
      if(reminder)reminder.insertAdjacentElement('beforebegin',card);else home.prepend(card);
    }
    return card;
  }
  ...
  function renderCard(){
    ...
    if(!date){card.innerHTML='<div class="attendance-head"><div><strong>Sunday Availability</strong><div class="muted">No upcoming Sunday game is posted yet.</div></div></div>';return}
    const name=playerName(),games=gamesFor(date),answer=name?responseFor(date,name):'';
    const gameText=games.map(g=>'<div><strong>'+time12(g.time)+'</strong> — '+esc((g.title||'Game').replace(/ \(Boston.*$/,''))+'</div>').join('');
    card.innerHTML=`<div class="attendance-head"><div><div class="muted">WEEKLY GAME CHECK-IN</div><h2 style="margin:.25rem 0">Will you be there Sunday?</h2><div>${esc(prettyDate(date))}</div></div><span class="attendance-badge">LINEUP RSVP</span></div>
      <div class="attendance-games">${gameText}</div>
      ${name?'<div><strong>'+esc(name)+', choose your answer:</strong></div>':'<div><strong>Choose your name first:</strong></div>'}
      ${name?`<div class="attendance-answer"><button data-answer="yes" class="${answer==='yes'?'on':''}">✅ Yes</button><button data-answer="no" class="no ${answer==='no'?'on':''}">❌ No</button><button data-answer="not_sure" class="maybe ${answer==='not_sure'?'on':''}">🤔 Not sure</button></div>`:`<div class="attendance-player"><select id="attendancePlayer">${playerOptions(name)}</select></div>`}
      ${answer?'<div class="attendance-saved">Saved: '+statusLabel(answer)+' • Captain View updates automatically.</div>':''}
      <div class="attendance-notify"><div id="attendanceNotifyText" class="muted">Turn on Thursday reminders so you do not have to remember to check the app.</div><button id="enableAttendancePush" style="margin-top:8px">Enable Thursday Push Reminder</button></div>`;

    const sel=document.getElementById('attendancePlayer');if(sel)sel.onchange=()=>setPlayer(sel.value);
    card.querySelectorAll('[data-answer]').forEach(btn=>btn.onclick=()=>saveAnswer(btn.dataset.answer));
  ...
  function b64ToBytes(value){
    const pad='='.repeat((4-value.length%4)%4),base64=(value+pad).replace(/-/g,'+').replace(/_/g,'/'),raw=atob(base64);return Uint8Array.from([...raw].map(c=>c.charCodeAt(0)));
  }
  ...
  async function syncExistingSubscription(){
    try{if(!playerName()||!('PushManager'in window))return;const reg=await registration(),sub=await reg.pushManager.getSubscription();if(sub)await postSubscription(sub)}catch(e){}
  }
  async function refreshPushStatus(){
    ...
    if(!('Notification'in window)||!('PushManager'in window)){text.textContent='This browser does not support web push reminders.';btn.disabled=true;return}
    const isIOS=/iphone|ipad|ipod/i.test(navigator.userAgent);
    if(isIOS&&!standalone()){text.textContent='On iPhone, add Bunt Cakes to your Home Screen first. Then open the app and enable notifications.';btn.textContent='Home Screen app required';btn.disabled=true;return}
    try{const reg=await registration(),sub=await reg.pushManager.getSubscription();if(sub&&Notification.permission==='granted'){text.textContent='Thursday push reminders are ON for this phone.';btn.textContent='Thursday Reminders On';btn.disabled=true;await postSubscription(sub);return}}catch(e){}
    text.textContent='Get a push notification each Thursday around 6 PM when a Sunday game is scheduled.';btn.textContent='Enable Thursday Push Reminder';btn.disabled=false;
  }
  async function enablePush(){
    const name=playerName();if(!name){alert('Choose your name first.');return}
