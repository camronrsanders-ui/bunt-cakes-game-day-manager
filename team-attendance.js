(function(){
  ...
  function gameDates(){const t=today(),events=Array.isArray(state?.events)?state.events:[];return[...new Set(events.filter(e=>e&&e.type==='Game'&&e.date&&e.date>=t).map(e=>e.date))].sort()}
  ...
  async function refreshPushStatus(){
    ...
    if(status.subscribed&&status.permission==='granted'){text.textContent='Game availability reminders are ON for this phone.';btn.textContent='✓ Thursday Notifications On';btn.disabled=true;return}
    ...
    text.textContent='One tap will ask your phone for permission, then Thursday reminders will be enabled.';btn.textContent='Turn On Game Reminders';btn.disabled=false;
  }
