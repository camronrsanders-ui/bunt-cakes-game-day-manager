(()=>{
  ...
  function mountSwitcher(){
    ...
    const add=document.createElement('button');add.type='button';add.textContent='+ New team';add.style.minHeight='38px';add.onclick=async()=>{const pilotCode=prompt('Enter your Founding Team invite code to create another Haus:');if(pilotCode===null)return;const clean=String(pilotCode||'').trim();if(!clean){alert('A Founding Team invite code is required during the pilot.');return}add.disabled=true;add.textContent='Creating…';try{const r=await jsonFetch('/api/captains',{method:'POST',body:JSON.stringify({action:'create-team',pilotCode:clean,timeZone:tz()})});location.href=r.captainUrl}catch(e){alert(e.message);add.disabled=false;add.textContent='+ New team'}};wrap.append(select,add);right.prepend(wrap);
  }
  ...
  function mountSlugEditor(){
    ...
    card.innerHTML=`<div class="row wrap"><div><strong>Team links</strong><div class="muted">Your team gets its own player and captain URLs. Owners can customize the link during setup; it locks after player access or Home Screen installs begin so existing player shortcuts do not break.</div></div><span class="pill">${esc(session?.team?.role||'captain')}</span></div><div class="grid g2" style="margin-top:9px"><label>Team link code<input id="teamSlugInput" value="${esc(current())}" ${session?.team?.role==='owner'?'':'disabled'}></label><div><div class="muted">Player link</div><code id="settingsPlayerLink">${esc(location.origin+'/team/'+current())}</code></div></div><button id="saveTeamSlug" type="button" ${session?.team?.role==='owner'?'':'disabled'} style="margin-top:9px">Save team link</button><div id="teamSlugMsg" class="muted"></div>`;
  ...
  function overrideAccess(){
    if(typeof window.renderAccess!=='function'||window.renderAccess.__tenantAware)return;const fn=async function(){setShareLinks();const list=document.getElementById('captainList');if(!list)return;try{const r=await jsonFetch('/api/captains');const canManage=r.viewerRole==='owner';list.innerHTML=r.captains.map(c=>`<div class="card row wrap"><div><strong>${esc(c.display_name)}</strong><div class="muted">${esc(c.email)} • ${esc(c.role)}</div></div>${c.role==='owner'?'<span class="pill">Owner</span>':canManage?`<button class="danger removeCaptain" data-email="${esc(c.email)}">Remove</button>`:'<span class="pill">Captain</span>'}</div>`).join('');list.querySelectorAll('.removeCaptain').forEach(b=>b.onclick=async()=>{if(!confirm('Remove this captain from this team?'))return;try{await jsonFetch('/api/captains',{method:'POST',body:JSON.stringify({action:'remove-member',email:b.dataset.email})});fn()}catch(e){alert(e.message)}})}catch(e){list.innerHTML='<div class="error">'+esc(e.message)+'</div>'}};fn.__tenantAware=true;window.renderAccess=fn;renderAccess=fn;
  }
