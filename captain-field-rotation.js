(function(){
  ...
  function currentInning(){return Math.max(1,Math.min(7,Number(state?.fieldInning||1)));}
  ...
  function fitScore(p,pos){
    ...
    const list=prefs(p),idx=list.indexOf(pos);
    if(idx>=0)return1000-(idx*80);
    if(isFlexible(p))return700;
    if(isWillingElsewhere(p))return250;
    return-5000;
  }
  ...
  function mount(){
    ...
    section.innerHTML=`
      <div class="card rotation-hero"><div class="rotation-title"><div><div class="muted">CAPTAIN TOOL</div><h2>Field Rotation</h2><div class="muted">Build each inning directly on the field. Player survey preferences stay visible while you assign.</div></div><div id="rotationProfileProgress" class="rotation-progress"></div></div><div id="rotationSurveyWarning" class="rotation-warning"></div></div>
      <div class="card"><div class="row wrap"><strong>Inning to edit</strong><span class="muted">Tap a field position to change it</span></div><div id="rotationInnings" class="inning-strip" style="margin-top:9px"></div></div>
      ...
      <div class="card rotation-actions"><button id="buildPreferenceRotation" class="primary">Build Rotation From Preferences</button><div class="rotation-note">This creates a draft across innings 1–7. It will not run automatically, and it will stop if any present player is still missing fielding preferences.</div></div>
  ...
  function renderInningStrip(){
    const box=document.getElementById('rotationInnings');if(!box)return;const n=currentInning();
    box.innerHTML=Array.from({length:7},(_,i)=>`<button type="button" data-inning="${i+1}" class="${n===i+1?'on':''}">${i+1}</button>`).join('');
    box.querySelectorAll('button').forEach(b=>b.onclick=()=>{state.fieldInning=Number(b.dataset.inning);renderFieldRotation();if(typeof renderLineup==='function')renderLineup();});
  }
  ...
  function renderEditor(){
    ...
    box.innerHTML=`<div class="muted">EDIT POSITION</div><h3 style="margin:.25rem 0">${esc(selectedPosition)}</h3><label>Player<select id="rotationPlayerSelect">${options}</select></label>${st?`<span class="fit-chip ${st.key}">${esc(st.label)}</span><div class="rotation-note" style="margin-top:7px">${esc(profileLabel(p))}</div>`:'<div class="rotation-note" style="margin-top:7px">Choose an available player. The list is sorted by survey fit.</div>'}`;
    document.getElementById('rotationPlayerSelect').onchange=e=>{inn[selectedPosition]=e.target.value;queueSave();renderFieldRotation();if(typeof renderLineup==='function')renderLineup();};
  }
  function renderRest(){
    ...
    box.innerHTML=`<div class="muted">RESTING THIS INNING</div><h3 style="margin:.25rem 0">${resting.length} player${resting.length===1?'':'s'}</h3><div class="rotation-list">${resting.length?resting.map(p=>`<div class="rotation-person"><strong>${esc(p.name)}</strong><span class="muted">${esc(isComplete(p)?(isFlexible(p)?'Flexible':(prefs(p)[0]||'Survey complete')):'Survey missing')}</span></div>`).join(''):'<div class="muted">Everyone is assigned.</div>'}</div>`;
  }
  function renderNext(){
  ...
  function buildPreferenceRotation(){
    ...
    const newInnings={};const stats=new Map(available.map(p=>[p.name,{played:0,last:0,byPos:{}}]));
    for(let inning=1;inning<=7;inning++){
      const out={};const used=new Set();
      const positions=Object.keys(COORDS).slice().sort((a,b)=>{
        const ca=available.filter(p=>fitScore(p,a)>-5000).length,cb=available.filter(p=>fitScore(p,b)>-5000).length;return ca-cb;
      });
      positions.forEach(pos=>{
        let best=null,bestScore=-Infinity;
        available.forEach(p=>{
          if(used.has(p.name))return;const fit=fitScore(p,pos);if(fit<=-5000)return;const s=stats.get(p.name);let score=fit-(s.played*145)-((s.byPos[pos]||0)*35);if(s.last===inning-1)score-=15;else score+=35;if(score>bestScore){bestScore=score;best=p;}
        });
        if(best){out[pos]=best.name;used.add(best.name);const s=stats.get(best.name);s.played++;s.last=inning;s.byPos[pos]=(s.byPos[pos]||0)+1;}
      });
      Object.keys(COORDS).forEach(pos=>{if(!(pos in out))out[pos]='';});newInnings[inning]=out;
    }
    state.innings=newInnings;queueSave();renderFieldRotation();if(typeof renderLineup==='function')renderLineup();alert('Preference-based 7-inning draft built. Review each inning on the field before game day.');
  }
