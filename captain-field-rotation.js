(function(){
  ...
  const submitted=p=>p?.surveySubmitted===true;
  ...
  function statusFor(p,pos){
    if(!p||!submitted(p))return{key:'missing',label:'Survey missing'};
    ...
    if(!prefs(p).length)return{key:'neutral',label:'No preference stated'};
    return{key:'outside',label:'Captain override'};
  }
  function fitScore(p,pos){
    if(!p||!submitted(p))return-10000;
    const i=prefs(p).indexOf(pos);
    if(i>=0)return 1000-(i*45);
    if(flexible(p))return 500;
    if(willing(p))return 150;
    if(!prefs(p).length)return 100;
    return-5000;
  }
  function profileLabel(p){
    if(!p||!submitted(p))return'Survey missing';
    const parts=[];if(prefs(p).length)parts.push(prefs(p).join(' → '));if(flexible(p))parts.push('Flexible / anywhere');if(willing(p))parts.push('willing elsewhere');return parts.join(' • ')||'Survey submitted • No position preference stated';
  }
  ...
  function mount(){
    ...
    section.innerHTML=`
      <div class="card rotation-hero"><div class="rotation-title"><div><div class="muted">CAPTAIN TOOL</div><h2>Field Rotation</h2><div class="muted">Survey preferences guide the builder. The captain can manually place any present player at any position when needed.</div></div><div id="rotationProfileProgress" class="rotation-progress"></div></div><div id="rotationSurveyWarning" class="rotation-warning"></div><div id="rotationDraftBanner" style="margin-top:10px"></div></div>
  ...
  function renderEditor(){
    ...
    box.innerHTML=`<div class="muted">${draftInnings?'EDIT DRAFT':'EDIT LIVE ROTATION'} • INNING ${currentInning()}</div><h3 style="margin:.25rem 0">${esc(selectedPosition)}</h3><label>Player<select id="rotationPlayerSelect">${options}</select></label>${st?`<span class="fit-chip ${st.key}">${esc(st.label)}</span><div class="rotation-note" style="margin-top:7px">${esc(profileLabel(p))}</div>`:'<div class="rotation-note" style="margin-top:7px">Captain override is always allowed. Choose any present player; if they are already assigned in this inning, they will be moved here automatically.</div>'}`;
    document.getElementById('rotationPlayerSelect').onchange=e=>{const next=e.target.value;if(next){Object.keys(inn).forEach(pos=>{if(pos!==selectedPosition&&inn[pos]===next)inn[pos]='';});}inn[selectedPosition]=next;if(!draftInnings)queueSave();renderFieldRotation();if(!draftInnings&&currentInning()===liveInning()&&typeof renderLineup==='function')renderLineup();};
  }
  function renderRest(){
    const box=document.getElementById('rotationRest');if(!box)return;const inn=inningData(currentInning()),used=new Set(Object.values(inn).filter(Boolean)),resting=presentPlayers().filter(p=>!used.has(p.name));box.innerHTML=`<div class="muted">RESTING • INNING ${currentInning()}</div><h3 style="margin:.25rem 0">${resting.length} player${resting.length===1?'':'s'}</h3><div class="rotation-list">${resting.length?resting.map(p=>`<div class="rotation-person"><strong>${esc(p.name)}</strong><span class="muted">${esc(submitted(p)?(flexible(p)?'Flexible':(prefs(p)[0]||'No preference stated')):'Survey missing')}</span></div>`).join(''):'<div class="muted">Everyone is assigned.</div>'}</div>`;
  }
  function renderNext(){
  ...
  function renderSurveySummary(){
    const list=presentPlayers(),received=list.filter(submitted),missing=list.filter(p=>!submitted(p)),progress=document.getElementById('rotationProfileProgress'),warning=document.getElementById('rotationSurveyWarning');if(progress)progress.textContent=`${received.length} of ${list.length} surveys submitted`;if(warning){warning.className='rotation-warning '+(missing.length?'':'rotation-good');warning.innerHTML=missing.length?`<strong>${missing.length} survey response${missing.length===1?' is':'s are'} still missing.</strong><div class="survey-missing-list">${missing.map(p=>esc(p.name)).join(', ')}</div>`:'<strong>All present players have submitted the survey.</strong>';}
    const profiles=document.getElementById('rotationProfiles');if(profiles)profiles.innerHTML=`<div class="row wrap"><div><strong>Survey & Fielding Preference Check</strong><div class="muted">A submitted survey counts even when the player did not choose a position.</div></div><span class="pill">${received.length}/${list.length} submitted</span></div><div style="margin-top:8px">${list.map(p=>`<details><summary>${esc(p.name)} ${submitted(p)?'✓':'⚠'}</summary><div class="muted">${esc(profileLabel(p))}</div></details>`).join('')}</div>`;
  }
  ...
  function buildPreferenceRotation(){
    const available=presentPlayers(),missing=available.filter(p=>!submitted(p));if(missing.length){alert('Rotation not built. Survey responses are still missing for: '+missing.map(p=>p.name).join(', '));return;}if(!available.length){alert('Rotation not built. No players are marked present.');return;}if(!confirm('Build a new preference-based seven-inning draft? Players with no stated preference may be placed where the team needs them. Nothing live will change until you publish it.'))return;
    ...
    draftInnings=newInnings;editInning=liveInning();renderFieldRotation();alert(gaps?`Draft built with ${gaps} unfillable field slot${gaps===1?'':'s'} based on the submitted survey data. Nothing live changed.`:'Preference-based seven-inning draft built with full field coverage. Nothing live changed until you publish it.');
  }
  function install(){
