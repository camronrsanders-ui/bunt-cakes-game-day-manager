(()=>{
  ...
  const FIELD_SVG=`<svg viewBox="0 0 400 440" aria-hidden="true" focusable="false">
  ...
  function renderFieldMap(line,label){
    return `<div class="assignment-map-card"><div class="assignment-map-label"><strong>${label}</strong><span class="muted">${label==='Layout 1'?'Primary rotation':'Second rotation'}</span></div><div class="assignment-map" role="group" aria-label="Assigned fielders for ${label.toLowerCase()}">${FIELD_SVG}${FIELD_SPOTS.map(([position,short,x,y])=>{
        const value=clean(line?.[position]);
        const name=rosterPlayer(value)?.name||value||'Unassigned';
        return `<div class="assignment-marker ${value?'':'is-empty'}" style="left:${x}%;top:${y}%" title="${esc(position+': '+name)}"><span class="assignment-position">${short}</span><span class="assignment-name" aria-label="${esc(position+': '+name)}">${esc(name)}</span></div>`;
      }).join('')}</div></div>`;
  }
  function renderFieldDiagram(){
    const box=document.getElementById('captainAssignmentDiagram');if(!box)return;
    const preview=dirty?buildInnings():null;
    const lines=[fieldLine(1,preview),fieldLine(2,preview)];
    const same=JSON.stringify(lines[0])===JSON.stringify(lines[1]);
    const extras=POSITIONS.map(pos=>{
      const names=normalizePosition(config(false)?.positions?.[pos]||blankPosition()).slots.slice(2).map(s=>clean(s.name)).filter(Boolean).map(display);
      return names.length?`${pos}: ${names.join(' / ')}`:'';
    }).filter(Boolean);
    box.innerHTML=`<div class="assignment-map-heading"><div><strong>Field layout</strong><div class="muted">${dirty?'Draft preview • tap Update live to share':'Assigned positions'}${same?' • same lineup across both layouts':''}</div></div></div><div class="assignment-map-grid">${renderFieldMap(lines[0],'Layout 1')}${renderFieldMap(lines[1],'Layout 2')}</div>${extras.length?`<div class="assignment-map-extra"><strong>Additional rotation slots</strong><span>${esc(extras.join(' • '))}</span></div>`:''}<div class="assignment-map-note">Layout 1 and Layout 2 show the first two rotation slots. Additional slots continue in later innings; field depth is schematic.</div>`;
  }
