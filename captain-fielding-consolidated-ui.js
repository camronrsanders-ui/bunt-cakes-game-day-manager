(()=>{
  ...
  let selectedInning=null,scheduled=false;

  const esc=v=>String(v??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const getState=()=>typeof state!=='undefined'&&state?state:null;
  const targetDate=()=>{
    try{return String(window.BuntGameDayEligibility?.targetDate?.()||'');}catch(_){return'';}
  };
  const isActive=p=>{
  ...
  function currentLine(s,inning){return s?.innings?.[inning]||s?.innings?.[String(inning)]||{};}
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      #fieldingInningPreview .fielding-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px 12px;margin-top:10px}
      #fieldingInningPreview .fielding-row{display:flex;justify-content:space-between;gap:8px;padding:5px 0;border-bottom:1px solid #eceff1}
      #fieldingInningPreview .fielding-pos{color:var(--m);font-size:.86rem}
      #fieldingInningPreview .fielding-player{font-weight:750;text-align:right}
      #fieldingInningPreview .fielding-rest{margin-top:10px;padding:9px 10px;background:#f9fafb;border-radius:10px}
      #fieldingInningPreview .fielding-next{margin-top:8px;font-size:.9rem;color:var(--m)}
  ...
  function panelHtml(s,inning){
    ...
    return `<div class="fielding-head"><div><strong>Fielding plan</strong><div class="muted">Who is on the field this inning</div></div><select id="fieldingPreviewInning">${options}</select></div><div class="fielding-grid">${rows}</div><div class="fielding-rest"><strong>Resting:</strong> ${resting.length?resting.map(esc).join(', '):'None'}</div><div class="fielding-next"><strong>Next inning:</strong> ${esc(nextChanges(s,inning))}</div>`;
  }
  function render(){
  ...
  document.addEventListener('click',e=>{
    if(e.target?.closest?.('#pods button'))setTimeout(schedule,150);
  });
  document.addEventListener('change',e=>{
    if(e.target?.closest?.('#pods'))setTimeout(schedule,80);
  });
  window.addEventListener('buntpreferrednamesrefresh',schedule);
  window.addEventListener('focus',schedule);
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedule();});
  const observer=new MutationObserver(()=>schedule());
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{observer.observe(document.body,{childList:true,subtree:true});schedule();},{once:true});
  else{observer.observe(document.body,{childList:true,subtree:true});schedule();}
})();
