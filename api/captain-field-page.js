...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    html = html.replace('</head>', '<script src="/tenant-context.js?v=2"></script><link rel="stylesheet" href="/captain-console-layout.css?v=3"></head>');
    html = html.replace(
    ...
    html = html.replace(
      ...
      `function defaultState(){const innings={};for(let i=1;i<=7;i++)innings[i]={};return{team:{name:'',shortName:'',organization:'',sport:'Kickball',location:'',primaryColor:'#15803d',accentColor:'#f7fff8',logoDataUrl:'',logoUrl:'',chatUrl:'',announcement:'',arrivalMinutes:60,secondReminderMinutes:30,leagueAppsEnabled:false,timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'},playerVisibility:{schedule:true,lineup:true,pods:true,kicking:true,officials:true,resources:true,attendance:true},resources:[],players:[],innings,pods:[],fieldSwitches:{},fieldSwitchInningOverrides:{},kickingOrder:[],score:{team:0,opponent:0},counts:{balls:0,strikes:0,fouls:0,outs:0},gameInning:1,fieldInning:1,half:'Team kicking',events:[],season:{name:'',division:'',color:'#15803d'},lastLeagueSync:null}}function migrate(s){`
    );

    // Keep the legacy Lineup section in the DOM because the upstream inline script
    // still binds to it, but remove its Captain navigation button entirely.
    html = html.replace('<button data-tab="lineup">Lineup</button>', '');
    html = html.replace('<button data-tab="pods">Pods</button>', '<button data-tab="pods">Fielding</button>');
    html = html.replace('<strong>Rotation Pods</strong>', '<strong>Fielding</strong>');
    html = html.replace('Group players who can rotate through the same positions. Set the pod, then build the 7-inning plan so everyone can see who swaps and who rests.', 'Assign roster players or game-only players to each field position and rotate them by inning.');
    html = html.replace('Add Pod', 'Auto Assign');
    html = html.replace('Build 7-Inning Rotation', 'Update live');
    ...
    html = html.replace('</body>', freshLeagueApps + '<script src="/field-profile-model.js?v=2"></script><script src="/captain-field.js?v=3"></script><script src="/captain-access.js?v=3"></script><script src="/captain-invite-access.js?v=1"></script><script src="/captain-kicking.js?v=2"></script><script src="/captain-live-sync.js?v=12"></script><script src="/captain-save-reliability.js?v=2"></script><script src="/captain-roles.js?v=1"></script><script src="/preferred-names.js?v=1"></script><script src="/captain-results.js?v=1"></script><script src="/captain-attendance.js?v=7"></script><script src="/captain-position-switch-loader.js?v=4"></script><script src="/captain-field-compact-ui.js?v=2"></script><script src="/captain-team-settings.js?v=2"></script><script src="/captain-tab-scope-fix.js?v=3"></script><script src="/captain-team-preview.js?v=2"></script><script src="/captain-tenant-ui.js?v=2"></script><script src="/team-officiating-view.js?v=2"></script><script src="/rules-visual-renderer.js?v=2"></script><link rel="stylesheet" href="/umpire-training-guide.css?v=1"><script src="/umpire-training-data.js?v=1"></script><script src="/umpire-training-guide.js?v=1"></script><script src="/umpire-console.js?v=4"></script><script src="/captain-officiating-rotation.js?v=3"></script><script src="/team-role-badges.js?v=1"></script><script src="/captain-account-controls.js?v=1"></script><script src="/captain-activity.js?v=1"></script></body>');
