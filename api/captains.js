...
function pilotInviteConfigured(){
  return normalizePilotCode(process.env.FEILDHAUS_PILOT_CODE).length>=6;
}
function pilotInviteMatches(value){
  const expected=normalizePilotCode(process.env.FEILDHAUS_PILOT_CODE);
...
async function signup(req,res,sql){
  ...
  if(!pilotInviteConfigured()) return res.status(503).json({error:'Founding Team enrollment is temporarily closed'});
  if(!pilotInviteMatches(pilotCode)) return res.status(403).json({error:'A valid Founding Team invite code is required'});
...
module.exports = async function handler(req,res){
  try{
    ...
    if(req.method==='GET'&&String(req.query&&req.query.action||'')==='pilot-status') return res.status(200).json({pilot:true,inviteRequired:true,enrollmentOpen:pilotInviteConfigured()});
    ...
    if(req.method==='POST'&&action==='create-team'){
      if(!pilotInviteConfigured()||!pilotInviteMatches(req.body&&req.body.pilotCode)) return res.status(403).json({error:'A valid Founding Team invite code is required'});
