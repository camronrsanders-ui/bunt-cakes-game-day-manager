(()=>{
  ...
  function bind(){
    ...
    Object.entries(fields).forEach(([id,key])=>{const el=$id(id);if(!el)return;el.value=t[key]||'';el.onchange=()=>{const raw=el.value.trim();if(id==='teamChat'&&raw&&!safeExternalUrl(raw)){el.setCustomValidity('Enter a full link starting with https:// or http://');el.reportValidity();return}if(id==='teamChat')el.setCustomValidity('');t[key]=raw;if(key==='name'||key==='shortName'){syncHalfOptions(t,true);applyBrand(t)}queueSave();renderSettingsSummary()}});
