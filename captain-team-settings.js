(()=>{
  ...
  function renderResources(){const box=$id('teamSettingsResources');if(!box||typeof state==='undefined'||!state)return;box.innerHTML=(state.resources||[]).map(resourceRow).join('')||'<div class="card muted">No player resources yet.</div>';box.querySelectorAll('[data-resource]').forEach(row=>{const i=+row.dataset.resource;row.querySelector('.r-title').onchange=e=>{state.resources[i].title=e.target.value.trim();queueSave()};row.querySelector('.r-url').onchange=e=>{const input=e.target,raw=input.value.trim();if(raw&&!safeExternalUrl(raw)){input.setCustomValidity('Enter a full link starting with https:// or http://');input.reportValidity();return}input.setCustomValidity('');state.resources[i].url=raw;queueSave()};row.querySelector('.r-desc').onchange=e=>{state.resources[i].description=e.target.value.trim();queueSave()};row.querySelector('.r-remove').onclick=()=>{state.resources.splice(i,1);queueSave();renderResources()}})}
  function bind(){
    ...
    Object.entries(fields).forEach(([id,key])=>{const el=$id(id);if(!el)return;el.value=t[key]||'';el.onchange=()=>{const raw=el.value.trim();if(id==='teamChat'&&raw&&!safeExternalUrl(raw)){el.setCustomValidity('Enter a full link starting with https:// or http://');el.reportValidity();return}if(id==='teamChat')el.setCustomValidity('');if(id==='teamTimezone'){try{new Intl.DateTimeFormat('en-US',{timeZone:raw}).format(new Date());el.setCustomValidity('')}catch(_){el.setCustomValidity('Enter a valid IANA time zone, such as America/New_York');el.reportValidity();return}}t[key]=raw;if(key==='name'||key==='shortName'){syncHalfOptions(t,true);applyBrand(t)}queueSave();renderSettingsSummary()}});
