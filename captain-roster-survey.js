(function(){
  ...
  function decorate(){
    ...
    [...box.children].forEach(card=>{
      ...
      details.querySelectorAll('.survey-position input').forEach(cb=>cb.onchange=()=>{const checked=new Set([...details.querySelectorAll('.survey-position input:checked')].map(x=>x.value));const old=Array.isArray(player.preferences)?player.preferences:[];const next=old.filter(pos=>checked.has(pos));POS.forEach(pos=>{if(checked.has(pos)&&!next.includes(pos))next.push(pos)});player.preferences=next;queueSave();card.dataset.surveyEnhanced='';renderRoster();});
