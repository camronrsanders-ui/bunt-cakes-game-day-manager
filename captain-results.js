(function(){
  ...
  function mount(){
    ...
    card.querySelector('#saveFinalResult').onclick=()=>{
      ...
      const row={key,date:e.date,title:shortTitle(e),teamScore:Number(state.score?.team||0),opponentScore:Number(state.score?.opponent||0),savedAt:new Date().toISOString()};
      const idx=state.gameResults.findIndex(r=>r.key===key);if(idx>=0)state.gameResults[idx]=row;else state.gameResults.push(row);
      queueSave();mount();
    };
    card.querySelector('#clearLiveScore').onclick=()=>{if(!state.score)state.score={team:0,opponent:0};state.score.team=0;state.score.opponent=0;queueSave();if(typeof renderDash==='function')renderDash();mount();};
    card.querySelectorAll('[data-delete-result]').forEach(b=>b.onclick=()=>{state.gameResults=state.gameResults.filter(r=>r.key!==b.dataset.deleteResult);queueSave();mount();});
  }
